...
describe("App e2e", () => {
  ...
  describe("/users", () => {
    ...
    describe("cursor pagination", () => {
      ...
      it("GET /users/cursor with after parameter paginates forward", async () => {
        ...
        const firstPage = await request(host)
          .get("/users/cursor")
          .query({ limit: "1" });
        ...
        if (firstPage.body.pageInfo.hasNextPage) {
          const secondPage = await request(host)
            .get("/users/cursor")
      ...
      it("GET /users/cursor with before parameter paginates backward", async () => {
        ...
        if (firstPage.body.pageInfo.hasNextPage) {
          ...
          const previousPage = await request(host)
            .get("/users/cursor")
  ...
  describe("/post/with-comment (transactions)", () => {
    let userId: string;
